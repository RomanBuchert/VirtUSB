...
Backend implementations interact with the VirtUSB architecture as illustrated below.

```mermaid
flowchart LR
    OS["Operating System"] --> HC["VirtUSB Host Controller"]
    HC --> UC["Userspace Component"]
    UC --> BI["Backend Interface"]
    BI --> BE["Backend"]
    BE --> DEV["Virtual USB Device"]
```
...
The following lifecycle illustrates that creation/removal of a virtual device
instance is independent of its attachment to a virtual USB port.
